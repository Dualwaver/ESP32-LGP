...
// char value[50],
char text[] = "Hello world from ESP32";
...
class MyCharacteristicCallbacks: public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic *customCharacteristic){
    ...
    if(rcvString.length()>0){
      ...
      //   value[i] = 0;/*NULL*/
     customCharacteristic->setValue((char*)&text);
    }
    else
      Serial.println("Empty Value Received!");
  }

  void onRead(BLECharacteristic *customCharacteristic){
    std::string rcvString = customCharacteristic->getValue();
    Serial.println("\nValue Sended from BLE: ");
    for (int i = 0; i < rcvString.length(); ++i){
      Serial.print(rcvString[i]);
        // value[i]=rcvString[i];
    }
...
void setup() {
  ...
  pCharacteristic->setValue((char*)&text);
  pService->start();
